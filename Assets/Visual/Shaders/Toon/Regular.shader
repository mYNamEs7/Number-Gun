Shader "Toony/Default"
{
	...
	SubShader
	{
		...
		inline half4 LightingToonyColorsCustom (SurfaceOutputCustom s, half3 lightDir, half3 viewDir, half atten)
		{
			...
			fixed ndl = max(0, dot(s.Normal, lightDir)*0.5 + 0.5);
			...
			#else
				fixed3 ramp = half3(1, 1, 1) * smoothstep(_RampThreshold - _RampSmoothing * 0.5, _RampThreshold + _RampSmoothing * 0.5, ndl);
			#endif
				...
				half specularIntensity = smoothstep(0.5 - _SpecularSmooth, 0.5 + _SpecularSmooth, pow(NdotH * ramp, _SpecularIntensity));
				half3 specular = specularIntensity * _SpecularColor.rgb * _SpecularColor.a;
			#endif

			#if HIGHLIGHT
				half3 highlight = step(_HighlightScale, dot(lightDir, s.Normal)) * _HighlightColor.rgb * _HighlightColor.a;
			#endif
